...
public class CsvService : ICsvService
{
    ...
    public CsvValidationResult ValidateCsv(string csvContent)
    {
        ...
        try
        {
            ...
            while (csv.Read())
            {
                ...
                try
                {
                    ...
                    else if (seenSafeKeys.Contains(safeKey))
                    {
                        errors.Add(new CsvError 
                        { 
                            LineNumber = lineNumber, 
        ...
        {
            _logService.Log($"CSV 驗證失敗: {ex.Message}", Models.LogLevel.Error);
            errors.Add(new CsvError { LineNumber = 0, Message = $"CSV 格式錯誤: {ex.Message}" });
            return new CsvValidationResult { IsValid = false, Errors = errors };
        }
    }
}
