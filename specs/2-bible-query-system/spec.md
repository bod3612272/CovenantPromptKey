...
3. **Given** 使用者選擇錯誤的書卷，**When** 確認答案，**Then** 系統顯示正確答案並將此錯題加入錯題記錄
4. **Given** 使用者完成 10 題，**When** 遊戲結束，**Then** 系統顯示本次分數、歷史最高分及最近 5 次遊戲記錄
5. **Given** 使用者查看遊戲記錄，**When** 選擇清除記錄，**Then** 系統清除所有遊戲歷史資料
6. **Given** 使用者查看錯題記錄，**When** 開啟錯題複習功能，**Then** 系統顯示最近 5 次遊戲中所有答錯的題目，包含經文內容、使用者選擇的答案及正確答案
7. **Given** 錯題記錄已滿（最多保留 5 次遊戲的錯題），**When** 新遊戲產生新的錯題，**Then** 系統移除最舊一次遊戲的錯題記錄
...
- **FR-056**: 系統 MUST 記錄使用者最近 5 次遊戲中所有答錯的題目，包含經文內容、經文出處、使用者選擇的答案及正確答案
- **FR-057**: 系統 MUST 提供「錯題複習」功能，讓使用者查看並學習先前答錯的題目
- **FR-058**: 系統 MUST 於錯題記錄超過 5 次遊戲時，自動移除最舊一次遊戲的錯題記錄
- **FR-059**: 系統 MUST 允許使用者單獨清除錯題記錄，而不影響遊戲分數記錄
...
- **SC-010**: 錯題記錄正確保留最近 5 次遊戲中所有答錯的題目，並可供使用者複習
